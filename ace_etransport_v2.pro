...
pro ace_etransport_v2




  ; my attempt at writin the transport code
  ;two stream theory
  mu=acos(60*!dtor) ;cosine of the angle between the path and the magnetic field


  ;output variables
  upflux=dblarr(jmax,nbins)
  downflux=dblarr(jmax,nbins)

  zzcm=zz*1.e5  ;change the altitudes into cm
  ;first create the T1 and T2 terms as function of altitude
  ;T1=sum over wavelength(bs*sig_es*ns)  in units of cm-1
  ;T2=sum over wavelength(bs*sig_es*ns+sig_is*ns)
  ...
  eiionz_transp_wv=fltarr(jmax,nmaj,nbins)
  ;eiionz_ht=fltarr(nbins) ;height integrated ionisation rates
  ...
  for i=0,jmax-1 do begin  ;altitude
    ...
    for j=0,nmaj-1 do begin  ;species
      ...
      for l=0, nbins-1 do begin
        ;      eiionz_transp_wv(i,j,l)=total(tflux[i,l]*del[l]*sigix[*,j,l]*zmaj[j,i])
        ...
        for k=0,nei-1 do begin  ;states
          ...
          ;      eiionzk_transp[k,j,i]= total(tflux[i,*]*del*sigix[k,j,*])*zmaj[j,i]
